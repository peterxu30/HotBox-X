...
//		minObstacleWidth = minWidth;
	...
	public ArrayList<Obstacle_OLD> createObstacleWave(boolean zoned) {
//		if (zoned) { fix later
...
//		}
		...
		ArrayList<Obstacle_OLD> wave = new ArrayList<Obstacle_OLD>();
	...
	private void checkOverlap(ArrayList<Obstacle_OLD> wave) {
		
	}
	
//Obstacle(float x, float y, float width, float height, float ms, String obstacleImg)
	private Obstacle_OLD createObstacle(float x, float y, float width) {
		Random dimensionsRandom = new Random();
		return new Obstacle_OLD(x, y, width, obstacleHeight, movementSpeed, obstacleImage);
	}
